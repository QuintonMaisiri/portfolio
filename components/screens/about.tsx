...
const ReturnFileType = ({
  detail,
  onSelect,
  handleFolderToggle,
  expandedFolders,
}: {
  detail: FileType | { name: string; color: string; files: FileType[] };
  handleFolderToggle: (folderName: string) => void;
  expandedFolders: string[];
  onSelect: (file: FileType) => void;
}) => {
  if (typeof detail === "object" && "files" in detail) {
    return (
      <div>
        <div
        ...
        >
          <ChevronDown
            className={`w-[16px] transition-transform ${
              expandedFolders.includes(detail.name) ? "rotate-0" : "-rotate-90"
            }`}
          />
    ...
    return (
      <div
        onClick={() => onSelect(detail as FileType)}
  ...
  return (
    <div>
      ...
      <div className="lg:hidden flex flex-col h-[82vh] w-full overflow-y-hidden">
            ...
            {aboutMeData.categories.map((c, index) => {
              ...
              return (
                <div>
                  ...
                  <div
                    className={`${open && selectedCategory.name == c.name ? "block" : "hidden"}`}
                  >
        ...
        <div className="flex-1 overflow-y-scroll ">{selectedFile && <div className="p-5">
            ...
            <div className="flex flex-col gap-5">
              {aboutMeData.categories.map((c, index) => (
                <Image
                  key={index}
                  src={
                    selectedCategory === c ? c.iconSelected : c.iconUnselected
                  }
                  alt={c.name}
                  width={24}
                  height={24}
